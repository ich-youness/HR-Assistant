...
const Chat = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      message: "Hello! I'm your HR Assistant. I can help you with your onboarding.",
      isUser: false,
...
const handleSendMessage = async (message: string) => {
  ...
  try {
    // ✅ CALLING /voice instead of /chat
    const response = await fetch("http://localhost:5000/text", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ transcript: message }), // ✅ use 'transcript' here
    });
    ...
    const botMessage: Message = {
      ...
      message: data.response,  // assuming your backend returns: { "response": "text from Retell agent" }
      isUser: false,
      timestamp: new Date().toISOString(),
    };
